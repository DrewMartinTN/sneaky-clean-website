...
function bookingCard(v){
  const saved=state.bookings.find(b=>b.vehicle===v.index),deposit=activeDeposit(v.index),credited=deposit?deposit.amountCents-(deposit.payment?.refundedCents??0):0,cancelled=saved?.booking&&['CANCELLED_BY_CUSTOMER','CANCELLED_BY_SELLER','DECLINED','NO_SHOW'].includes(saved.booking.status);
