...
async function refresh(){await perform(async()=>{let next=await api('status');if(next.state==='processing')next=await api('quote');if(JSON.stringify(next)!==JSON.stringify(state)){state=next;renderState();}});}
async function init(){
  try{const r=await fetch(ESTIMATE_API+'/estimates/api/config');config=await r.json();if(!r.ok)throw Error(config.error||'Service pricing is temporarily unavailable. Please call or text us.');if(token){state=await api('status');renderState();if(state.state==='processing'){state=await api('quote');renderState();}}else renderForm();
...
init();
setInterval(()=>{if(token&&state?.input&&!busy&&!document.hidden&&(['review','processing'].includes(state.state)||state.needsPhotoReview||state.pendingQuote)&&!bookSelections.size)refresh();},30000);
