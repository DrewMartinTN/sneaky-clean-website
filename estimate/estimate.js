...
const TAG_PARAMS={estimate_start:[],generate_lead:[],estimate_quoted:['value','currency'],quote_accepted:['value','currency'],appointment_booked:['value','currency','transaction_id'],click_call_now:[],conversion:['value','currency','transaction_id']};
...
function renderForm(){
  const promo=promoActive();progress(0);
  app.innerHTML=`${promo?promoBanner():''}<form id="estimate-form" method="post" action="${esc(location.pathname)}"><div id="vehicles">${vehicleCard(0)}</div><button type="button" id="add-vehicle" class="secondary add-vehicle">＋ Add another vehicle</button><section class="card"><label for="notes">Other notes or timing preferences <span>(optional)</span></label><textarea id="notes" name="notes" maxlength="${3000-(promo?PROMO_SUMMARY_ROOM:0)}" placeholder="Anything that would help us prepare…"></textarea></section><section class="card contact-card"><div class="card-heading"><div><span class="section-num">Almost done</span><h2>Where should we send your exact price?</h2></div></div><p class="hint">If we can match your vehicle, your itemized quote appears on the next screen. We only contact you about this request.</p><div class="grid">${field('Full name','name','text','autocomplete="name" maxlength="200"')}${field('Mobile phone','phone','tel','autocomplete="tel" maxlength="40"')}${field('Email','email','email','autocomplete="email" maxlength="254"')}<div><label for="contactMethod">Preferred contact</label><select id="contactMethod" name="contactMethod">${options([['text','Text me'],['call','Call me'],['email','Email me']],'text')}</select></div><div class="full">${field('Service address','address','text','autocomplete="street-address" maxlength="300" placeholder="Street, city, state & ZIP"')}</div></div><div class="trap" aria-hidden="true"><label for="website">Website</label><input name="website" id="website" tabindex="-1" autocomplete="off"></div></section><div class="submit-bar"><label class="check consent"><input type="checkbox" id="consent" required><span>I agree to Sneaky Clean using these details and photos to prepare my quote${config.automaticPhotoReview?", including automated photo review":""}, and contacting me about this request using my preferred method. This does not sign me up for marketing messages.</span></label><div class="actions"><button type="submit" class="primary" id="submit">Get my estimate →</button><p class="hint">Your quote is free. A 50% deposit reserves your appointment after you choose a time.</p></div></div></form>`;
  const form=document.getElementById('estimate-form');form.onsubmit=e=>{e.preventDefault();sendForm();};
  form.addEventListener('focusin',e=>progress(e.target.closest?.('.contact-card,.submit-bar')?1:0));
  ...
  restoreDraft();
  try{const saved=localStorage.getItem('sneaky-estimate');if(!token&&/^[a-f0-9]{64}$/.test(saved??''))app.insertAdjacentHTML('afterbegin',`<p class="hint">Already requested a quote? <a href="#${saved}" data-private>Open your saved estimate</a>.</p>`);}catch{}
}
...
function inputData(){
  ...
  return {name:get('name'),phone:get('phone'),email:get('email'),address:get('address'),contactMethod:get('contactMethod'),notes:tagNotes(picks.join('; '),get('notes'),3000,'your other notes'),vehicles,consent:document.getElementById('consent').checked,website:get('website')};
}
function restoreDraft(){
  ...
  for(const key of ['name','phone','email','address','contactMethod','notes'])document.getElementById(key).value=key==='notes'&&promo?untagNotes(draft.notes).text:draft[key];
...
async function sendForm(){
  ...
  try{
    ...
    message('Saving your request…');state=await api('submit',data);if(state?.id)once('sc-tag-lead-'+state.id,false,()=>track('generate_lead'));try{localStorage.setItem('sneaky-estimate',token);}catch{}sessionStorage.removeItem('estimate-'+token);renderState();state=await api('quote');renderState();message(state.quote?'Your quote is ready. Photo review is still pending.':'Your request is saved. The team will confirm your vehicle and quote.');
...
function progress(step){for(const [i,key]of ['vehicle','details','quote','book'].entries()){const el=document.getElementById('step-'+key);if(!el)continue;el.classList.toggle('active',i<=step);if(i===step)el.setAttribute('aria-current','step');else el.removeAttribute('aria-current');}}
